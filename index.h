...
<head>
  ...
  <style>
    ...
    input { background:#0f1522; color:#fff; border:1px solid #334; border-radius:8px; padding:8px; }
  </style>
</head>
<body>
  ...
  <div class="tabs">
    ...
    <button id="tab-cred">Credential</button>
  ...
  <div id="panel-cred" class="panel">
    ...
    <div class="card">
      <label><input id="staEnabled" type="checkbox" /> Enable STA</label>
      <h4>STA Mode</h4>
    ...
    <div class="card">
      <label><input id="apEnabled" type="checkbox" /> Enable AP</label>
      <h4>AP Mode</h4>
      <input id="apSsid" placeholder="AP SSID" style="width:100%;" />
      <input id="apPassword" placeholder="AP Password" type="password" style="width:100%;margin-top:8px;" />
      <p><input id="apChannel" type="number" min="1" max="13" value="1" /> Channel</p>
      <label><input id="apBroadcast" type="checkbox" checked /> Broadcast SSID</label>
      <p><small>Jika STA+AP aktif, channel AP mengikuti channel STA.</small></p>
    </div>
  </div>
...
function setCredCardState(){
  ...
  const ap = $('apEnabled').checked;
  ['staSsid','staPassword','btnScan'].forEach(id => $(id).disabled = !sta);
  ['apSsid','apPassword','apChannel','apBroadcast'].forEach(id => $(id).disabled = !ap);
}
$('staEnabled').addEventListener('change', setCredCardState);
$('apEnabled').addEventListener('change', setCredCardState);

function applyCredentialToForm(c){
  $('staEnabled').checked = !!c.staEnabled;
  $('apEnabled').checked = !!c.apEnabled;
  $('staSsid').value = c.staSsid || '';
  $('staPassword').value = c.staPassword || '';
  $('apSsid').value = c.apSsid || '';
  $('apPassword').value = c.apPassword || '';
  $('apChannel').value = c.apChannel || 1;
  $('apBroadcast').checked = c.apBroadcast !== false;
  setCredCardState();
}
...
function collectCredential(){
  return {
    staEnabled: $('staEnabled').checked,
    apEnabled: $('apEnabled').checked,
    staSsid: $('staSsid').value,
    staPassword: $('staPassword').value,
    apSsid: $('apSsid').value,
    apPassword: $('apPassword').value,
    apChannel: Number($('apChannel').value || 1),
    apBroadcast: $('apBroadcast').checked,
  };
}
...
$('btnScan').addEventListener('click', async () => {
  ...
  $('scanResults').innerHTML = html || '<small>Tidak ada SSID ditemukan</small>';
  [...$('scanResults').querySelectorAll('button[data-ssid]')].forEach(btn => {
